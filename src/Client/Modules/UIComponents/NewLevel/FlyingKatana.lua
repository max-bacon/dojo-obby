...
local path = Waypoints.new(UDim2.fromScale(-0.3,.7), UDim2.fromScale(.5,-.1), UDim2.fromScale(1.3,.7))
...
local function flyingAnimInit(katanaIns)
	...
	print(path)
	local SpinTween = TweenService:Create(katanaIns, TweenInfo.new(2, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, -1), {Rotation = 360})
	...
	katanaIns.Position = UDim2.fromScale(-0.3,.7)
...
local function katana(props)
	...
	local katanaIns = New("ImageLabel")({
		...
		Position = UDim2.fromScale(-0.3,.7),
